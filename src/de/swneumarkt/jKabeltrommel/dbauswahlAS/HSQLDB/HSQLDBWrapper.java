...
//                System.out.println(getResultSetAsStringTable(rs));
        ...
        public boolean createTrommel(IKabeltypE kabelTyp, String trommelnummer, int gesamtlaenge, String lagerPlatz, int start, ILieferantE lieferantE, long lieferdatum, String lieferscheinNr) {
                try {
                        if (!(kabelTyp != null && trommelnummer != null && lagerPlatz != null && lieferantE != null && lieferscheinNr != null && gesamtlaenge >= 0)) {
                                ...
                                ResultSet liefer = getResultSet("SELECT * FROM lieferant WHERE hid=" + lieferantE.getId() + " AND name='" + lieferantE.getName() + "';");
                                // Lieferant nicht enthalten
                                if (!liefer.next()) {
                                        return false;
                                } else {
                                        liefer.close();
                                        ResultSet rs = getResultSet("insert into trommel(id, materialnummer,trommelnummer,gesamtlaenge,lagerplatz,kabelstart) VALUES(NULL," + kabelTyp.getMaterialNummer() + ",'" + trommelnummer + "', " + gesamtlaenge + ",'" + lagerPlatz + "'," + start + "); CALL IDENTITY()");
                                        if (!rs.next()) {
                                                return false;
                                        } else {
        ...
        public boolean createLieferant(String name) {
                ...
                try {
                        ResultSet rs = getResultSet("SELECT * FROM lieferant WHERE name='" + name + "' ;");
